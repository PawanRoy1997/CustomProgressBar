...
class GoalProgressBar(context: Context?, attrs: AttributeSet) : View(context, attrs) {
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            setGoal(state.getInt("goal"))
            val superState: Parcelable = state.getParcelable("superState") ?: Bundle()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val height: Int = when (MeasureSpec.getMode(heightMeasureSpec)) {
            ...
            MeasureSpec.AT_MOST -> goalIndicatorHeight.coerceAtMost(specHeight.toFloat()).toInt()
    ...
    private fun setGoalIndicatorHeight(goalIndicatorHeight: Float) {
        this.goalIndicatorHeight = goalIndicatorHeight
        postInvalidate()
    }

    private fun setGoalIndicatorThickness(goalIndicatorThickness: Float) {
        this.goalIndicatorThickness = goalIndicatorThickness
        postInvalidate()
    }

    private fun setGoalReachedColor(goalReachedColor: Int) {
        this.goalReachedColor = goalReachedColor
        postInvalidate()
    }

    private fun setGoalNotReachedColor(goalNotReachedColor: Int) {
        this.goalNotReachedColor = goalNotReachedColor
        postInvalidate()
    }

    private fun setUnfilledSectionColor(unfilledSectionColor: Int) {
        this.unfilledSectionColor = unfilledSectionColor
        postInvalidate()
    }

    private fun setBarThickness(barThickness: Int) {
        this.barThickness = barThickness
        postInvalidate()
    }

    private fun setIndicatorType(indicatorType: IndicatorType) {
